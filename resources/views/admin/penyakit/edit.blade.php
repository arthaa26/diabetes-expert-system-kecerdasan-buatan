...
<div class="min-h-screen bg-gray-100 p-6">
    <div class="max-w-2xl mx-auto">
        ...
        <div class="mb-6">
            ...
            <h1 class="text-3xl font-bold text-gray-800 mt-2">Edit Penyakit</h1>
        </div>
            ...
            <form action="{{ route('admin.penyakit.update', $penyakit) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="deskripsi" rows="3"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500">{{ old('deskripsi', $penyakit->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    <textarea name="penanganan" rows="4"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500">{{ old('penanganan', $penyakit->penanganan) }}</textarea>
                    @error('penanganan')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
